...
import { fetchIgnitionReport } from "@/lib/api"
import { VehicleAlert } from "@/lib/types"
import { Zap, PowerOff, RefreshCw, Bell, type LucideIcon } from "lucide-react"
import { toast } from "sonner"
import { ReportFilters } from "./ReportFilters"
import { ReportPagination } from "./ReportPagination"

const TYPE_STYLE: Record<"IGNITION_ON" | "IGNITION_OFF", string> = {
    IGNITION_ON: "bg-green-50 text-green-700 border-green-200",
    IGNITION_OFF: "bg-slate-100 text-slate-600 border-slate-200",
}

const TYPE_LABEL: Record<"IGNITION_ON" | "IGNITION_OFF", string> = {
    IGNITION_ON: "Ignition On",
    IGNITION_OFF: "Ignition Off",
}

const TYPE_ICON: Record<"IGNITION_ON" | "IGNITION_OFF", LucideIcon> = {
    IGNITION_ON: Zap,
...
export function IgnitionReportPanel() {
    ...
    const load = useCallback(async (p: number, size: number, from: Dayjs, to: Dayjs, veh: string) => {
        setLoading(true)
        try {
            const data = await fetchIgnitionReport(
            ...
            console.error("Failed to load ignition report", e)
            toast.error("Failed to load ignition report")
        } finally {
            setLoading(false)
        }
    }, [])
        ...
        <div className="flex flex-col h-full bg-gradient-to-b from-slate-50 to-slate-100/60">
            <div className="shrink-0 px-6 pt-6 pb-5 border-b border-slate-200 bg-white space-y-4">
                <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-yellow-50 text-yellow-600 ring-1 ring-yellow-100">
                        <Zap className="w-5 h-5" />
                    </div>
                    <div>
                        <h2 className="text-lg font-bold text-slate-900 tracking-tight leading-tight">Ignition Report</h2>
                        <p className="text-xs text-slate-400">Ignition on/off events with timestamps</p>
                    </div>
                </div>
                <ReportFilters
                    vehicleNo={vehicleNo}
            ...
            <div className="flex-1 min-h-0 flex flex-col px-6 py-4">
                {!hasRun ? (
                    <div className="flex flex-col items-center justify-center h-40 gap-3 text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200">
                        <div className="p-3 bg-slate-50 rounded-full"><Zap size={24} className="text-slate-400" /></div>
                    ...
                    <div className="flex flex-col items-center justify-center h-40 gap-3 text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200">
                        ...
                        <p className="text-sm font-medium">No ignition events found for this range</p>
                    </div>
                ) : (
                    <div className="flex-1 min-h-0 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="flex-1 min-h-0 overflow-y-auto">
                            {entries.map((a, idx) => {
                                const type = a.type as "IGNITION_ON" | "IGNITION_OFF"
            ...
            <ReportPagination
                ...
                loading={loading} idPrefix="ignition-report"
