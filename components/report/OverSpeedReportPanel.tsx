...
import { fetchOverSpeedReport } from "@/lib/api"
...
const COLS = "220px 200px 130px 130px minmax(0,1fr)"
const DEFAULT_PAGE_SIZE = 25

export function OverSpeedReportPanel() {
    ...
    const load = useCallback(async (p: number, size: number, from: Dayjs, to: Dayjs, veh: string) => {
        setLoading(true)
        try {
            const data = await fetchOverSpeedReport(
            ...
            console.error("Failed to load over speed report", e)
            toast.error("Failed to load over speed report")
        } finally {
            setLoading(false)
        }
    }, [])
        ...
        <div className="flex flex-col h-full bg-gradient-to-b from-slate-50 to-slate-100/60">
            <div className="shrink-0 px-6 pt-6 pb-5 border-b border-slate-200 bg-white space-y-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-slate-900 tracking-tight leading-tight">Over Speed Report</h2>
                        <p className="text-xs text-slate-400">Over-speed events with actual speed vs. the vehicle&apos;s limit</p>
                    </div>
                </div>
                <ReportFilters
                    vehicleNo={vehicleNo}
            ...
            <div className="flex-1 min-h-0 flex flex-col px-6 py-4">
                {!hasRun ? (
                    <div className="flex flex-col items-center justify-center h-40 gap-3 text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200">
                        <div className="p-3 bg-slate-50 rounded-full"><AlertTriangle size={24} className="text-slate-400" /></div>
                    ...
                    <div className="flex flex-col items-center justify-center h-40 gap-3 text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200">
                        ...
                        <p className="text-sm font-medium">No over-speed events found for this range</p>
                    </div>
                ) : (
                    <div className="flex-1 min-h-0 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="grid border-b border-slate-200 bg-slate-50/80 text-left text-[11px] uppercase tracking-wide text-slate-400" style={{ gridTemplateColumns: COLS }}>
                            ...
                            <div className="px-4 py-3 font-semibold">Speed</div>
                            <div className="px-4 py-3 font-semibold">Limit</div>
                            ...
                            {entries.map((a, idx) => (
                                ...
                                >
                                    ...
                                    <div className="px-4 py-3 self-center">
                                        <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full border bg-red-50 text-red-700 border-red-200">
                                            <AlertTriangle size={11} />
                                            {a.alertValue != null ? `${a.alertValue.toFixed(0)} km/h` : "—"}
                                        </span>
                                    </div>
                                    <div className="px-4 py-3 text-slate-600 text-sm self-center">
                                        {a.alertLimit != null ? `${a.alertLimit.toFixed(0)} km/h` : "—"}
                                    </div>
            ...
            <ReportPagination
                ...
                loading={loading} idPrefix="over-speed-report"
