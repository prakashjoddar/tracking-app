...
import { saveTrip, saveStops, fetchUsers, finishActiveTrip } from "@/lib/api";
...
export default function TripForm() {
  ...
  const [disableSequenceStallTimeout, setDisableSequenceStallTimeout] =
  ...
  const { vehicles } = useVehicleManageStore();
  const vehicleNo =
    vehicles.find((v) => v.id === selectedVehicleId)?.number ?? null;
  const activeTrip = useActiveTrip(vehicleNo);
  const [finishing, setFinishing] = useState(false);
  const [justFinished, setJustFinished] = useState(false);

  useEffect(() => {
    setJustFinished(false);
  }, [vehicleNo, activeTrip?.tripId]);

  const handleFinishActiveTrip = async () => {
    if (!vehicleNo) return;
    try {
      setFinishing(true);
      await finishActiveTrip(vehicleNo);
      setJustFinished(true);
      toast.success("Trip is finishing — it'll close within a few seconds.");
    } catch (e: any) {
      console.error("Finish active trip failed:", e);
      toast.error(e.response?.data?.message || "Failed to finish trip.");
    } finally {
      setFinishing(false);
    }
  };
  ...
  useEffect(() => {
    if (editingTrip) {
      ...
      setSequenceLookahead(
        editingTrip.sequenceLookahead != null
          ? String(editingTrip.sequenceLookahead)
          : "",
      );
      setDisableSequenceStallTimeout(
        editingTrip.disableSequenceStallTimeout ?? false,
      );
      ...
      setSequenceLookahead("");
      setDisableSequenceStallTimeout(false);
  ...
  const handleSave = async (): Promise<void> => {
    try {
      const payload = {
        ...
        followSequence,
        sequenceLookahead:
          followSequence && sequenceLookahead.trim()
            ? parseInt(sequenceLookahead, 10)
            : null,
        disableSequenceStallTimeout: followSequence
          ? disableSequenceStallTimeout
          : null,
        killAtMidnight,
        forceKillHours,
      ...
      // Only an existing trip can legitimately carry over stops here — a
      // freshly created trip has none yet (stops are managed separately via
      // "Edit Stops"), and saving `stops` unconditionally is what let a
      // stale, previously-loaded trip's stops get re-parented onto a new one.
      if (editingTrip && stops.length > 0) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Active trip banner */}
      {activeTrip && !justFinished && (
        <div className="shrink-0 px-6 py-3 border-b bg-amber-50 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-amber-700 min-w-0">
            ...
            <span className="truncate">
              <strong>{activeTrip.tripName}</strong> is currently running on
              this vehicle
            </span>
          </div>
          <button
            onClick={handleFinishActiveTrip}
            disabled={finishing}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-amber-600 text-white hover:bg-amber-700 disabled:opacity-50 transition-colors shrink-0"
          >
            <PowerOff size={12} />
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <Section icon={<Clock size={13} />} title="Schedule">
          <FormField label="Trip Timings">
            <div className="flex items-center gap-3">
              <TimePicker
                ...
                value={startTime ? dayjs(startTime, FORMAT) : null}
                format={FORMAT}
                className="flex-1"
                needConfirm={false}
                onChange={(_, val) => setStartTime(val as string)}
              />
              <span className="text-xs text-gray-400">to</span>
              <TimePicker
                placeholder="End time"
                value={endTime ? dayjs(endTime, FORMAT) : null}
                format={FORMAT}
                className="flex-1"
                needConfirm={false}
                onChange={(_, val) => setEndTime(val as string)}
              />
            </div>
          </FormField>
          ...
          <FormField label="Repeat Same Day ">
            ...
            <p className="text-[11px] text-gray-400 mt-1">
              ...
              same day. On: it can start fresh again the same day after
              finishing.
            </p>
          </FormField>
        </Section>
        ...
        <Section icon={<ListOrdered size={13} />} title="Stop Sequence">
          ...
          {followSequence && (
            <FormField label="Disable Stall Timeout ">
              <Radio.Group
                options={[
                  { label: "Off", value: false },
                  { label: "On", value: true },
                ]}
                value={disableSequenceStallTimeout}
                optionType="button"
                buttonStyle="solid"
                onChange={(e) => setDisableSequenceStallTimeout(e.target.value)}
              />
              <p className="text-[11px] text-gray-400 mt-1">
                Off (default): a stop stuck at the head of the sequence window is
                auto-skipped after the stall timeout so the trip doesn't hang. On:
                that auto-skip never fires for this trip — it waits indefinitely
                for the stop to be reached.
              </p>
            </FormField>
          )}
          <p className="text-[11px] text-gray-400">
            When on, stops are only confirmed in route order — prevents a later
            stop (e.g. on the opposite lane of an earlier road) from being
            falsely marked arrived out of turn.
          </p>
        </Section>
        ...
        {driversLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2 text-gray-400">
            ...
            <p className="text-[11px] text-center text-gray-300">
              Add drivers from the Driver &amp; Supervisor page first.
            </p>
          </div>
        ) : (
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400 font-normal">
                {selectedSupervisorIds.length} selected
              </p>
            </div>
          </div>
        }
      ...
      >
        ...
        {supervisorsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2 text-gray-400">
            ...
            <p className="text-[11px] text-center text-gray-300">
              Add supervisors from the Driver &amp; Supervisor page first.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
