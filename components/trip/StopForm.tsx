...
type FieldError = Partial<
  Record<
    keyof Omit<Stop, "id" | "snapToRoute" | "studentId" | "tripId">,
    string
  >
>;
...
export default function StopForm({ setShowMap }: StopFormProps) {
  ...
  const {
    ...
    setLoading: setStudentsLoading,
  } = useStudentStore();

  const editingStop = stops.find((s) => s.id === editingStopId) ?? null;

  const [form, setForm] =
    useState<Omit<Stop, "id" | "snapToRoute" | "studentId" | "tripId">>(EMPTY);
  ...
  const validate = (): boolean => {
    ...
    if (!form.latitude || !form.longitude)
      e.latitude = "Coordinates are required";
    setErrors(e);
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <Section icon={<Navigation size={13} />} title="Coordinates">
          ...
          <FormField
            ...
            hint="How close the bus must get to confirm arrival"
          >
            <Input
              type="number"
              min={1}
